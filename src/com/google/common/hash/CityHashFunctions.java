...
public class CityHashFunctions {  
  ...
  public static final HashFunction CITY_HASH_64 = CityHash64Seedless.INSTANCE;

  public static class CityHash64Seedless extends AbstractNonStreamingHashFunction implements
      Serializable {
    private static final HashFunction INSTANCE = new CityHash64Seedless();

    @Override
    public HashCode hashBytes(byte[] input, int off, int len) {
      return HashCodes.fromLong(cityHash64(input, off, len));
    }
  ...
  public static HashFunction cityHash64Seeded(long seed) {
    return cityHash64Seeded(K2, seed);
  }

  public static HashFunction cityHash64Seeded(long seed0, long seed1) {
    return new CityHash64Seeded(seed0, seed1);
  }

  private static class CityHash64Seeded extends AbstractNonStreamingHashFunction implements
      Serializable {
    private final long seed0;
    private final long seed1;

    private CityHash64Seeded(long seed0, long seed1) {
      this.seed0 = seed0;
      this.seed1 = seed1;
    }
    ...
    @Override
    public HashCode hashBytes(byte[] input, int off, int len) {
      return HashCodes.fromLong(
          hashLen16(cityHash64(input, off, len) - seed0, seed1));
    }
  ...
  public static long[] cityHash128(byte[] src, int off, int len) {
    checkPositionIndexes(off, off + len, src.length);
    if (len >= 16) {
  ...
  public static HashFunction CITY_HASH_128 = CityHash128Seedless.INSTANCE;
  
  private static HashCode fromLongs(long[] longs) {
    ByteBuffer buf = ByteBuffer.allocate(Long.SIZE * longs.length).order(ByteOrder.LITTLE_ENDIAN);
    for (long l : longs) {
      buf.putLong(l);
    }
  ...
  public static class CityHash128Seedless extends AbstractNonStreamingHashFunction
      implements Serializable {
    private static final HashFunction INSTANCE = new CityHash128Seedless();

    @Override
    public HashCode hashBytes(byte[] input, int off, int len) {
      return fromLongs(cityHash128(input, off, len));
    }
  ...
  public static HashFunction cityHash128Seeded(long seed0, long seed1) {
    return new CityHash128Seeded(seed0, seed1);
  }

  public static class CityHash128Seeded extends AbstractNonStreamingHashFunction
      implements Serializable {
    private final long[] seeds;

    private CityHash128Seeded(long seed0, long seed1) {
      this.seeds = new long[] { seed0, seed1 };
    }

    @Override
    public HashCode hashBytes(byte[] input, int off, int len) {
      checkPositionIndexes(off, off + len, input.length);
      return fromLongs(cityHash128WithSeed(input, off, len, seeds));
    }
